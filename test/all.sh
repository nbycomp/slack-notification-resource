...
test update_message | jq -e '
  .webhook_url == "https://slack.com/api/chat.update" and
  .body.text == "Updated message" and
  .body.channel == "C1234567890" and
  .body.ts == "1405894322.002768"
'

test update_message_file | jq -e '
  .webhook_url == "https://slack.com/api/chat.update" and
  .body.text == "Updated message" and
  .body.channel == "C1234567890" and
  .body.ts == "1405894322.002768"
...
test text | jq -e "
  ...
  .body.link_names == false and
  .body.username == $(echo $username | jq -R .) and
  .body.text == \"Inline static \`text\`\" and
  .body.attachments == null and
  ( .body | keys | contains([\"channel\",\"icon_emoji\",\"icon_url\",\"username\",\"link_names\",\"text\",\"attachments\"]) ) and
  ( .body | keys | length == 7 )"
...
test text_file_empty_suppress | jq -e "
  ...
  ( .version | keys == [\"channel\", \"thread_ts\"] )"

test metadata | jq -e "
  ( .version | keys == [\"channel\", \"thread_ts\"] )        and
  ...
  ( .metadata[3].name == \"text\" )             and ( .metadata[3].value == \"Inline static text\" ) and
